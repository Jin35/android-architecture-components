package com.android.example.github.di;

import com.android.example.github.api.AuthorizedGithubService;
import com.android.example.github.auth.AuthParameters;
import com.android.example.github.auth.AuthorizedAuthInterceptor;
import com.android.example.github.repository.RepoRepository;
import com.android.example.github.ui.user.AuthorizedRepoProvider;
import com.android.example.github.ui.user.RepoProvider;
import com.android.example.github.util.LiveDataCallAdapterFactory;

import dagger.Module;
import dagger.Provides;
import okhttp3.OkHttpClient;
import retrofit2.Retrofit;
import retrofit2.converter.gson.GsonConverterFactory;


@Module(includes = AuthorizedActivityModule.class)
class AuthorizedModule {

    @AuthorizedScope
    @Provides
    @Authorized
    OkHttpClient provideOkHttpClient(AuthorizedAuthInterceptor authInterceptor) {
        return new OkHttpClient.Builder().addInterceptor(authInterceptor).build();
    }

    @Provides
    @AuthorizedScope
    @Authorized
    AuthorizedGithubService provideGithubService(@Authorized OkHttpClient client) {
        return new Retrofit.Builder()
                .baseUrl("https://api.github.com/")
                .client(client)
                .addConverterFactory(GsonConverterFactory.create())
                .addCallAdapterFactory(new LiveDataCallAdapterFactory())
                .build()
                .create(AuthorizedGithubService.class);
    }

    @Provides
    @AuthorizedScope
    RepoProvider provideRepoProvider(RepoRepository repoRepository, AuthParameters parameters,
                                     @Authorized AuthorizedGithubService githubService) {
        return new AuthorizedRepoProvider(repoRepository, parameters, githubService);
    }
}
